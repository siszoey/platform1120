...
function DrwInfo(data,flag) {
    ...
    if (flag == "view") {
        if (data.data.type == "ROCKPOINT") {
          ...
          drwInfox = layer.open({
        ...
        } else if (data.data.type == "PROFILE" ||  data.data.type == "PROBESLOT" || data.data.type == "DRILLHOLE") {//查看剖面
            if (drwInfox != null) {
                layer.close(drwInfox);
            } 
            setTimeout(() => {
            ...
            //$.ajax({
            //    url: servicesurl + "/api/RockDesign/getModelurl", type: "get", data: { "windowName": data.data.title },
            ...
            //        }
            //        else {//加载模型
            //            var modelurl = datasurl + "/SurModel" + datas;
            //            //删除上一个模型（保证只有一个模型）
            //            if (curtileset != null) {
            //                viewer.scene.primitives.remove(curtileset);
            //                viewer.scene.primitives.remove(curtileset);
            ...
            //                            if (layers[i].children[j].checked = true) {
        ...
        } else if ( data.data.type == "ROCKAREA") {
            console.log(data);
            drwInfox = layer.open({
                type: 1
                , title: ['信息查看', 'font-weight:bold;font-size:large;font-family:Microsoft YaHei']
        ...
        } else if (data.data.type == "ROCKLINE" ||data.data.type == "ROCKLINEFA") {
            console.log(data);
            drwInfox = layer.open({
                type: 1
                , title: ['信息查看', 'font-weight:bold;font-size:large;font-family:Microsoft YaHei']
        ...
        postionviewtables.reload({ id: 'postionviewids', data: data.data.data });
    } else if (flag == "update") {
